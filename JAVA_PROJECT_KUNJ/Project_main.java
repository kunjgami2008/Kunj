...
public class Project_main
{
    ...
    static BankUser[] users = new BankUser[100];
    static int accountNumber = 300010;
    ...
    public static void registerUser()
    {
        ...
        {
            if(users[i].u_name.equals(uname))
            {
        ...
        {
            System.out.println(
                "Account cannot be created. Minimum opening balance is Rs.10000."
            );
            return;
        }

        users[count] = new BankUser();

        if(typ.equalsIgnoreCase("Saving"))
        {
            users[count].account = new SavingAccount();
        }
        else
        {
            users[count].account = new CurrentAccount();
        }

        accountNumber++;

        users[count].account.setAccountnum(accountNumber);
        users[count].account.setBalance(balances);
        users[count].account.setOwner(users[count]);

        users[count].name = tname;
        users[count].u_name = uname;
        users[count].password = pass;
        users[count].gender = gen;
        users[count].age = ages;
        users[count].email = emails;
        users[count].mobileno = mn;
        users[count].adhar = adharno;
        users[count].pan = panno;
        users[count].address = addres;
        users[count].acctype = typ;
    ...
    public static void login()
    {
        ...
        for(int i = 0; i < count; i++)
        {
            if(users[i].u_name.equals(loginusername) &&
               users[i].password.equals(loginpassword))
            {
                currentUser = i;
    ...
    public static void depositMoney()
    {
        ...
        double currentbalance =
            users[currentUser].account.getBalance();

        users[currentUser].account.setBalance(
    ...
    public static void withdrawMoney()
    {
        ...
        users[currentUser].account.withdraw(amount);
    }
    ...
    public static void viewAccountDetails()
    {
        ...
        System.out.println(
            "UserName        : " + users[currentUser].u_name
        );
        ...
        System.out.println(
            "Mobile No       : " + users[currentUser].mobileno
        );

        System.out.println(
            "Aadhar Number   : " + users[currentUser].adhar
        );

        System.out.println(
            "PAN Number      : " + users[currentUser].pan
        );

        System.out.println(
            "Address         : " + users[currentUser].address
        );

        System.out.println(
            "Account Type    : " + users[currentUser].acctype
        );

        System.out.println(
            "Account Number  : "
            + users[currentUser].account.getAccountnum()
        );
